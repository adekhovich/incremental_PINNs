...
def reaction_solution(u0_str: str, u0, rho, t0=0, T=1, nx=256, nt=100):
    L = 2*np.pi
    dx = L/nx
    dt = (T-t0)/nt
    x = np.arange(0, 2*np.pi, dx)
    t = np.linspace(t0, T, nt).reshape(-1, 1)
    X, T = np.meshgrid(x, t)

    # call u0 this way so array is (n, ), so each row of u should also be (n, )
    if u0 is None:
        u0_func = function(u0_str)
        u0 = u0_func(x)

    u = reaction(u0, rho, T-t0)

    u = u.flatten()
    return u

def reaction_diffusion_discrete_solution(u0_str : str, u0, nu, rho, t0=0, T=1, nx = 256, nt = 100):
...
def convection_diffusion(u0_str: str, u0, nu, beta, source=0, t0=0, T=1, xgrid=256, nt=100):
    ...
    uhat0 = np.fft.fft(u0)
    nu_factor = np.exp(nu * IKX2 * (T-t0) - beta * IKX * (T-t0))
    A = uhat0 - np.fft.fft(G)*t0 # at t=0, second term goes away
    uhat = A*nu_factor + np.fft.fft(G)*(T-t0) # for constant, fft(p) dt = fft(p)*T
